    ...
    public class ApplicantProfileLogic : BaseLogic<ApplicantProfilePoco>
    {
        ...
        protected override void Verify(ApplicantProfilePoco[] pocos)
        {
            ...
            foreach (var poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException(111, $"Currency for Applicant Profile ID {poco.Id} is less than zero"));
                }

                if (poco.CurrentRate < 0)
                {
                    exceptions.Add(new ValidationException(112, $"Current Rate for Applicant Profile ID {poco.Id} is less than zero"));
                }
            }
